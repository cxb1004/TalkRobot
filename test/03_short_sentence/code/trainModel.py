"""
读取corpus_yyyymmdd.csv文件，训练模型
1、读取文件，读取内容文本，生成词向量
2、按日期生成临时模型目录，把生成的模型保存在这个目录下
"""
...
from common.config import Config as ProjConfig
...
# 模型训练基础数据目录：所有模型训练用到的数据，都会放在这个目录下，该目录以日期为名
base_data_path = ROOT_PATH + '/53data/' + industry_name + '/' + cur_date + '/'

# 语料库文件，读取，用以生成词向量文件
corpus_file = os.path.join(base_data_path, 'corpus.txt')
# 词向量文件
word2vec_file = os.path.join(base_data_path + 'word2Vec.bin')
# 词库word2idx.json文件路径
word2idx_json_file = os.path.join(base_data_path + 'word2idx.json')
# 标签库label2idx.json文件路径
label2idx_json_file = os.path.join(base_data_path + 'label2idx.json')

# 输出文件的路径
output_file_path = os.path.join(base_data_path + 'output')
...
class Config(object):
    ...
    # 当日的整理出来的打过标签的数据
    dataSource = os.path.join(ROOT_PATH, '53data/' + cur_date + '/labeledTrainData.csv')
    if not os.path.exists(dataSource):
        log.error('训练数据不存在，请检查：' + str(dataSource))
        exit(999)

    # 停用词文件：以分行的形式
    stopWordSource = os.path.join(ROOT_PATH, "53data/stopword")

    # 二分类设置为1，多分类设置为类别的数目
    global label_dict
    numClasses = len(label_dict)

    # 训练集的比例, 这个比例是把80%的数据来用于训练模型，20%的数据来验证模型
    rate = 0.8
...
class Dataset(object):
    ...
    def _readData(self, filePath):
        ...
        # 这里是把标签从原始数据文件中提取出来
        # 数据文件中：
        # 第一列：文本内容（没有经过清洗过滤，包含有html标签、标点符号等）
        # 第二列：标签ID   1～6
...
if not os.path.exists(corpus_file):
    log.error('语料库文件不存在：' + corpus_file)
    exit(999)
if os.path.exists(word2vec_file):
    os.remove(word2vec_file)
    log.info('词向量文件已经存在，自动删除')
...
log.info('词向量文件生成：' + str(word2vec_file))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        outDir = os.path.join(output_file_path, "summarys")
        ...
        savedModelPath = os.path.join(output_file_path, 'model/bilstm-atten/saveModel')
        ...
        for i in range(config.training.epoches):
            ...
            for batchTrain in nextBatch(trainReviews, trainLabels, config.batchSize):
                ...
                if currentStep % config.training.checkpointEvery == 0:
                    ...
                    my_model_path = os.path.join(output_file_path, 'model/Bi-LSTM-atten/model/my-model')
